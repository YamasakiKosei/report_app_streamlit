...
sheet_name = 'Original'                              # 雛形Excelのシート名
eval_ctg = ['電気的安全性点検', '機能点検']            # 総合評価の評価項目
ctg = ['電気的安全性点検', '外装点検', '機能点検']      # 点検項目
...
outi2 = stObject('外装点検', '外装点検２', '２．アクティブコネクタ、マイクロバイポーラコネクタ、\n\n　　対極板接続コネクタが清拭されているか')
outi3 = stObject('外装点検', '外装点検３', '３．フットスイッチ及びコードに汚れや破損がないか')
outi4 = stObject('外装点検', '外装点検４', '４．メス先電極及びコネクタ、対極板コード及びコネクタの破損がないか')
outi5 = stObject('外装点検', '外装点検５', '５．メス先電極ホルダ、ハンドコントロールスイッチの破損がないか')
outi6 = stObject('外装点検', '外装点検６', '６．電源コードの腐食や破損がないか')

fni1 = stObject('機能点検', '機能点検１', '１．電源投入時に各表示ランプ、指示ランプが点灯するか')
fni2 = stObject('機能点検', '機能点検２', '２．切開、凝固、バイポーラの各表示ランプ、指示ランプが点灯するか')
fni3 = stObject('機能点検', '機能点検３', '３．フットスイッチ接続コネクタ、マイクロバイポーラコネクタ、\n\n　　対極板接続コネクタ、アクティブコネクタのあそびなど異常がないか')
fni4 = stObject('機能点検', '機能点検４', '４．フットスイッチの機能に異常がないか')
fni5 = stObject('機能点検', '機能点検５', '５．対極板アラームが鳴るか')
fni6 = stObject('機能点検', '機能点検６', '６．出力設定に異常がないか')
...
outi2.bool = st.checkbox(outi2.label) # アクティブコネクタ、マイクロバイポーラコネクタ、対極板接続コネクタが清拭されているか
outi3.bool = st.checkbox(outi3.label) # フットスイッチ及びコードに汚れや破損がないか
outi4.bool = st.checkbox(outi4.label) # メス先電極及びコネクタ、対極板コード及びコネクタの破損がないか
outi5.bool = st.checkbox(outi5.label) # メス先電極ホルダ、ハンドコントロールスイッチの破損がないか
outi6.bool = st.checkbox(outi6.label) # 電源コードの腐食や破損がないか
st.divider()

# 機能点検
st.subheader('機能点検')
fni1.bool = st.checkbox(fni1.label) # 電源投入時に各表示ランプ、指示ランプが点灯するか
fni2.bool = st.checkbox(fni2.label) # 切開、凝固、バイポーラの各表示ランプ、指示ランプが点灯するか
fni3.bool = st.checkbox(fni3.label) # フットスイッチ接続コネクタ、マイクロバイポーラコネクタ、対極板接続コネクタ、アクティブコネクタのあそびなど異常がないか
fni4.bool = st.checkbox(fni4.label) # フットスイッチの機能に異常がないか
fni5.bool = st.checkbox(fni5.label) # 対極板アラームが鳴るか
fni6.bool = st.checkbox(fni6.label) # 出力設定に異常がないか
st.divider()
